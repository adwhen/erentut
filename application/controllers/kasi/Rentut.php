...
class Rentut extends CI_Controller {
	...
	public function tambah($id=null)
	{
		...
		$jumlah = $this->db->order_by('id_sop','DESC')->get_where('tb_sopform',array('posisi'=>$this->session->userdata('level'),'tujuan' => $this->session->userdata('keterangan_pegawai')))->result_array();
			...
			$jaksa = $this->db->get_where('tb_rentut',array('tb_rentut.id_sop' => $id,'level' => 1))->result_array();
			$data=array(
			...
			if($proses=0){
				$this->Mlog->logAktivitas("Kasi ".$this->session->userdata('username')." menambahkan usulan");
				$this->session->set_flashdata('msg','Usulan Berhasil Ditambahkan.');
				redirect('kasi/rentut/index');
			}else{
				$this->Mlog->logAktivitas("Kasi ".$this->session->userdata('username')." mengubah usulan");
				$this->session->set_flashdata('msg','Usulan Berhasil Diubah.');
				redirect('kasi/rentut/usulan');
			}
		}
	...
	public function form(){
		$jumlah = $this->db->order_by('id_sop','DESC')->get_where('tb_sopform',array('posisi'=>$this->session->userdata('level'),'tujuan' => $this->session->userdata('keterangan_pegawai')))->result_array();
		$data=array(
				'isi'=> 'kasi/rentut/manajemen_form',
				'data' => $this->db->order_by('id_sop','DESC')->
